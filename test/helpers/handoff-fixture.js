...
export function verifiedReturnFixture(target, packet, options = {}) {
  ...
  const repositoryEvidence = {
    ...
    changedPaths,
    checks,
    attribution: roleReturn.attribution,
    pr: roleReturn.pr,
  };
...
export function handoffExpectation(fixture, overrides = {}) {
  return {
    ...
    roleId: 'engineer',
    taskContractDigest: fixture.packet.task.contractDigest,
...
export function syntheticRecognizedVerdict(target, transition, options = {}) {
  ...
  const packet = {
    ...
    task: { id: taskId, digest: taskDigest, contractDigest },
    activation: null,
  ...
  const roleReturn = createRoleReturn({
    ...
    producerRole: 'engineer',
    packet: { packetId: packet.packetId, digest: packet.digest },
    task: { backend: 'files', id: taskId, digest: taskDigest },
    worktree: target,
    branch: 'task/T-001',
    head,
    baseHead,
    ...
    attribution: { range: { base: baseHead, head }, commits: [head] },
    pr: { state: 'not_applicable', number: null, url: null },
  ...
  const record = createReturnVerification({
    ...
    packet,
    roleReturn,
    repositoryEvidence: { backend: 'files', worktree: target, task: { id: taskId, digest: taskDigest } },
