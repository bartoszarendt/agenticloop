...
export function validateConfig(repoRoot, options = {}) {
  ...
  if (hasToolkitSource) {
    ...
    validateCanonicalStopContract(assetLayout, errors);
  }
...
const STOP_ROUTING_SNIPPETS = [
  'normalize the supplied argument by trimming surrounding whitespace.',
  'If and only if it equals `stop` (case-insensitive), immediately follow',
  '`agenticloop/commands/stop.md` and return.',
  'Do not continue this start workflow.',
  '`stop now`, `stop-gap fix`',
  'Do not treat `exit` as an alias.',
];

function validateStopRouting(text, label, errors) {
  validateRequiredSnippets(label, text, STOP_ROUTING_SNIPPETS, errors, 'missing stop-routing contract');
  const routeIndex = text.indexOf('If and only if it equals `stop` (case-insensitive), immediately follow');
  const projectMapIndex = text.indexOf('Read `.agenticloop/project.md` first.');
  if (routeIndex !== -1 && projectMapIndex !== -1 && routeIndex > projectMapIndex) {
    errors.push(`${label}: stop routing must appear before the project-map read`);
  }
}

function validateCanonicalStopContract(assetLayout, errors) {
  const commandsDir = join(assetLayout.assetRoot, COMMANDS_SOURCE_DIRECTORY.replace(/^agenticloop\//, ''));
  const startPath = join(commandsDir, 'start.md');
  const stopPath = join(commandsDir, 'stop.md');
  ...
  if (!existsSync(stopPath)) {
    errors.push(`Canonical stop command missing: ${stopPath.replace(/\\/g, '/')}`);
    return;
  }

  try {
    const stopText = readFileSync(stopPath, 'utf-8');
    validateRequiredSnippets(
      stopPath.replace(/\\/g, '/'),
      stopText,
      [
        'current-conversation deactivation',
        'Do not authorize or initiate new Agentic Loop work',
        'A voluntary stop is not `blocked` or `needs_context`.',
        'Do not accept, close, merge, commit, push, delete branches, clean up worktrees,',
        'After this summary, do not automatically continue Agentic Loop',
      ],
      errors,
      'missing stop safety contract'
    );
  } catch (error) {
    errors.push(`${stopPath.replace(/\\/g, '/')}: ${error.message}`);
  }
}
