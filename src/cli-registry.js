    ...
    subcommands: {
      ...
      'verify-return': {
        ...
        options: [
          ...
          opt('repository-evidence', 'string', 'Repository evidence refetched and revalidated at the return boundary. Required for successful verification; only the host_receipt path authenticates it through the producer receipt.'),
          opt('repo', 'string', 'GitHub repository in owner/name form. Defaults to the authenticated current repository for the GitHub backend.'),
           opt('producer-receipt', 'string', 'Ed25519 host-adapter receipt verified against the packet-bound adapter/key from the fixed operator trust registry. Required when the effective policy minimum is host_receipt; omit in standard mode for an explicitly session_reported return.'),
