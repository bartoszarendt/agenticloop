...
/**
 * Mode B: the generated `.claude/commands/agenticloop.md` command and
 * `.claude/skills/agenticloop/SKILL.md` skill this adapter renders.
 */
export const CLAUDE_CODE_ACTIVATION_ADAPTER_ID = 'claude-code.command.arguments.v1';

/**
 * Mode A: the static `/agenticloop:start` command that `.claude-plugin/plugin.json`
 * registers directly from `commands/start.md`. It is a live activation surface,
 * not an inert template, so it carries its own typed identity - distinct from
 * Mode B because the host registers and substitutes it without this adapter
 * rendering anything. Defined beside the slot contract and re-exported here.
 */
export { CLAUDE_CODE_PLUGIN_ACTIVATION_ADAPTER_ID } from '../adapter-slots.js';
...
function renderClaudePublicSkill(skillReferenceMap, agentNames) {
  ...
  let body = fillUnsupportedActivationSlots(rawBody.trim(), {
    adapterId: CLAUDE_CODE_ACTIVATION_ADAPTER_ID,
    limitation: 'Claude Code command arguments are injected into model-visible command text, not a host-owned capture channel.',
...
function copyClaudeCodeCommand(destDir, outputDir) {
  ...
  writeFileSync(destPath, renderClaudeCommandContent(), 'utf-8');
...
/**
 * The single rendering of the canonical activation command for Claude Code.
 * Both the planner (init/setup/generate) and the direct generator consume
 * this, so the production surface cannot diverge from the fail-closed
 * unsupported-activation declaration.
 */
function renderClaudeCommandContent() {
  const source = readFileSync(CLAUDE_CODE_START_COMMAND, 'utf-8');
  const marker = '<!-- Generated by: agenticloop generate claude-code. Do not edit by hand. -->';
  const [frontmatter, rawBody] = splitFrontmatterForMarker(source);
  const body = fillUnsupportedActivationSlots(rawBody, {
    adapterId: CLAUDE_CODE_ACTIVATION_ADAPTER_ID,
    limitation: 'Claude Code command arguments are injected into model-visible command text, not a host-owned capture channel.',
    requestedInput: 'Requested task or context: use the current user request or selected task id as advisory context.',
  });
