...
export function renderOpencodeCommandMarkdown() {
  const source = loadOpencodeCommandBody();
  // Fail immediately when a canonical slot marker is absent or duplicated rather
  // than silently emitting a command with no capability declaration.
  const body = fillUnsupportedActivationSlots(source, {
    adapterId: OPENCODE_ACTIVATION_ADAPTER_ID,
    limitation: 'OpenCode positional substitution is prompt text and provides no proven lossless parser-owned byte artifact.',
    requestedInput: 'Requested task or context: host command input (advisory only; not activation proof).',
  });
  return [
    '---',
