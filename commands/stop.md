...
2. When the host exposes a safe interruption control, interrupt active Agentic
   ...
   indefinitely; report the still-running work and host limitation. An active
   Agentic Loop supervision controller counts as active Agentic Loop work: use
   its authenticated stop control when available and include the result in the
   final summary. Stopping only the controller does not replace the remaining
   checkpoint and deactivation steps in this contract.
